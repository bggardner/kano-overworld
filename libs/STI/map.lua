...
local Map        = {}
local layerBatchesTile = {}
local layerBatchesObject = {}
...
function Map:init(STI, path, plugins, ox, oy)
	...
	for i, tileset in ipairs(self.tilesets) do
		...
		tileset.image = self.sti.cache[formatted_path]
                tileset.imageId = formatted_path
...
-- @return nil
function Map:setLayer(layer, path, i)
        layer.num = i
...
function Map:refreshLayer(layer)
	if layer.type == "tilelayer" then
		self:setSpriteBatches(layer)
	elseif layer.type == "objectgroup" then
		self:setObjectSpriteBatches(layer)
...
function Map:discardLayer(layer)
	if layer.type == "tilelayer" then
		self:discardSpriteBatches(layer)
	elseif layer.type == "objectgroup" then
		self:discardObjectSpriteBatches(layer)
	end
end
...
function Map:setSpriteBatches(layer)
	...
	local batches = {
        ...
        layerBatchesTile[layer.num] = layerBatchesTile[layer.num] or batches
	...
	for y=sy, ey, iy do
		...
		for x=sx, ex, ix do
			...
			if tile then
				...
				local id = self.tilesets[tile.tileset].imageId

				layerBatchesTile[layer.num].data[id]         = layerBatchesTile[layer.num].data[id] or {}
				layerBatchesTile[layer.num].data[id][by]     = layerBatchesTile[layer.num].data[id][by] or {}
				layerBatchesTile[layer.num].data[id][by][bx] = layerBatchesTile[layer.num].data[id][by][bx] or newBatch(image, size)

				local batch = layerBatchesTile[layer.num].data[id][by][bx]
				local tx, ty
...
function Map:discardSpriteBatches(layer)
    local batches = layerBatchesTile[layer.num]
    if batches then
        for id, batches_tileset in pairs(layerBatchesTile[layer.num].data) do
            for y, batches_y in pairs(batches_tileset) do
                for x, batch  in pairs(batches_y) do
                    batch:clear()
                end
            end
        end
...
function Map:setObjectSpriteBatches(layer)
	...
	local th       = self.tileheight
	local batches  = {}
        layerBatchesObject[layer.num] = layerBatchesObject[layer.num] or batches
	...
	for _, object in ipairs(layer.objects) do
		if object.gid then
			...
			layerBatchesObject[layer.num][id] = layerBatchesObject[layer.num][id] or newBatch(image, 50)

			local batch = layerBatchesObject[layer.num][id]
...
function Map:discardObjectSpriteBatches(layer)
    local batches = layerBatchesObject[layer.num]
    if batches then
...
function Map:drawTileLayer(layer)
	...
	local bw = layerBatchesTile[layer.num].width
	local bh = layerBatchesTile[layer.num].height
	...
	for by=sy, ey, iy do
		for bx=sx, ex, ix do
			if bx >= 1 and bx <= mx and by >= 1 and by <= my then
				for _, batches in pairs(layerBatchesTile[layer.num].data) do
...
function Map:drawObjectLayer(layer)
	...
	for _, batch in pairs(layerBatchesObject[layer.num]) do
